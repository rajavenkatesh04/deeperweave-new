...
// Matches exactly what get_followers_with_status returns
...
export function FollowerList({ targetUserId, viewerId }: FollowerListProps) {
    ...
    const fetchPage = useCallback(async (pageOffset: number) => {
        ...
        try {
            ...
            if (error) {
                console.error('get_followers_with_status error:', error);
                return;
            }

            const fetched = (data ?? []) as FollowerRow[];
    ...
    const handleRemove = useCallback(async (followerId: string) => {
        // Optimistic remove
        setRows(prev => prev.filter(r => r.user_id !== followerId));
        ...
        if (error) {
            console.error('remove_follower error:', error);
            // Refetch to restore accurate state
            fetchPage(0);
            toast.error('Failed to remove follower.');
        }
    }, [supabase, targetUserId, fetchPage]);
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            {isOwnProfile ? (
                                <button
                                    ...
                                    className="text-xs text-zinc-400 hover:text-red-500 transition-colors px-2 py-1 rounded-md hover:bg-red-50 dark:hover:bg-red-950/30"
                                >
                                    Remove
                                </button>
                            ) : (
