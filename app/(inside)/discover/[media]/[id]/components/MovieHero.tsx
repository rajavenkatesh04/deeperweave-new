...
// 1. Import new types (Provider, Keyword)
import { Movie, Provider, Keyword } from '@/lib/types/tmdb';
...
// 2. Define the Interface to accept the new Server-Side filtered data
...
export function MovieHero({ media, certification, providers = [], keywords = [] }: MovieHeroProps) {
    const title = media.title;
    const showOriginalTitle = media.original_language !== 'en' && media.original_title !== title;
    const releaseYear = media.release_date?.split('-')[0] || 'N/A';

    const formatRuntime = (mins?: number) => mins ? `${Math.floor(mins / 60)}h ${mins % 60}m` : 'N/A';
    const formatMoney = (amount?: number) => (amount && amount > 0) ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0, notation: 'compact' }).format(amount) : 'N/A';

    const director = media.credits?.crew.find(c => c.job === 'Director')?.name || 'N/A';
    const hasBackdrop = !!(media.backdrop_path || media.images?.backdrops?.length);

    return (
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 pb-20">
            {/* STICKY HEADER */}
            <header className="border-b border-zinc-200 dark:border-zinc-800 bg-white/90 dark:bg-zinc-950/90 sticky top-0 z-20 backdrop-blur-sm">
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    <BackButton />
                    <div className="flex items-center gap-3">
                        <SaveButton itemType="movie" itemId={media.id} className="bg-zinc-100 dark:bg-zinc-900 p-2 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-800" />
                        <ShareButton />
                    </div>
                </div>
            </header>
            ...
            <main className="relative z-10 max-w-7xl mx-auto px-6 -mt-32 md:-mt-48 lg:-mt-64">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 mb-16">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {/* ✨ NEW: Streaming Providers Display */}
                        {providers.length > 0 && (
                    ...
                    <div className="lg:col-span-3 space-y-8 lg:mt-64">
                        <div className="space-y-3">
                            <div className="flex items-center gap-3 flex-wrap">
                                ...
                                {media.status && <Badge variant="secondary" className="rounded-sm uppercase text-[10px] tracking-wider">{media.status}</Badge>}
                                <Badge className="bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border-none rounded-sm uppercase text-[10px] tracking-wider">Movie</Badge>
                            </div>
                            ...
                            {showOriginalTitle && <p className="text-sm text-zinc-500 font-mono">Original: {media.original_title}</p>}
                        </div>

                        <div className="flex flex-wrap items-center gap-4 text-sm text-zinc-600 dark:text-zinc-400">
                            <span className="font-bold text-zinc-900 dark:text-zinc-100">{releaseYear}</span>
                            <span>·</span><span>{formatRuntime(media.runtime)}</span>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <Button asChild variant="outline" className="h-10 px-5 gap-2 font-semibold rounded-full bg-white dark:bg-zinc-900 border-zinc-300 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all">
                                <Link href={`/profile/reviews/create?id=${media.id}&type=movie`}><PlusIcon className="w-4 h-4" /> Log Entry</Link>
                            </Button>
                        </div>
                        ...
                        {/* MOVIE SPECIFIC DATA GRID */}
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-6 border-y border-zinc-200 dark:border-zinc-800">
                            <div className="col-span-1"><p className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mb-1">Director</p><p className="font-semibold text-sm">{director}</p></div>
                            <div className="col-span-1"><p className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mb-1">Budget</p><p className="font-medium text-sm">{formatMoney(media.budget)}</p></div>
                            <div className="col-span-1"><p className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mb-1">Revenue</p><p className="font-medium text-sm text-green-600 dark:text-green-500">{formatMoney(media.revenue)}</p></div>
                            <div className="col-span-1"><p className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold mb-1"><LanguageIcon className="w-3 h-3 inline mr-1"/> Language</p><p className="font-medium text-sm uppercase">{media.original_language || 'N/A'}</p></div>
                        </div>
                ...
                <div className="mb-24">
                    <CastCrewSwitcher cast={media.credits?.cast || []} crew={media.credits?.crew || []} />
                </div>
